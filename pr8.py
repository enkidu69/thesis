...
def calculate_row_based_metrics_with_lookback(df, horizon_days):
    """
    Applies user-specific logic for row-by-row classification:
    - Lookback: Sum of 'Predicted_Alert' for the PRIOR x lines (shift 1).
    - TP: Event > 0 AND Prior_Sum > 0
    - FN: Event > 0 AND Prior_Sum == 0
    - FP: Event == 0 AND Prior_Sum > 0
    - TN: Event == 0 AND Prior_Sum == 0
    
    Returns: DataFrame with new 'Result_Type' column.
    """
    df = df.copy()
    
    # Calculate Sum of PRIOR x lines (Strictly prior: shift 1, then roll)
    # min_periods=1 ensures we get values even at the start
    df['Prior_Alerts_Sum'] = df['Predicted_Alert'].shift(1).rolling(window=horizon_days, min_periods=1).sum().fillna(0)
    ...
    # TP: Event present AND Alert was active in prior window
    df.loc[mask_event & mask_alert_active, 'Result_Type'] = 'TP'
    
    # FN: Event present BUT No Alert in prior window
    df.loc[mask_event & mask_no_alert, 'Result_Type'] = 'FN'
    
    # FP: No Event BUT Alert was active in prior window
    df.loc[mask_no_event & mask_alert_active, 'Result_Type'] = 'FP'
    
    # TN: No Event AND No Alert in prior window
...
def calculate_coverage_metrics(y_true_daily, y_pred_alerts, horizon_days):
    """
    Standard Coverage Metric Calculator used during optimization.
    """

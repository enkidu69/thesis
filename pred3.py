...
# Add this function to apply models with manual threshold
...
# -------------------------
# RUN MODELS WITH MANUAL THRESHOLD
# -------------------------

print("\n" + "="*80)
print("RUNNING MODELS WITH MANUAL THRESHOLD")
print("="*80)
print(f"Using manual threshold: {TIER_THRESHOLD}")

# Apply all models with manual threshold
manual_predictions = apply_models_with_manual_threshold(trained_models, test_df, feature_cols, TIER_THRESHOLD)

# Merge with original test data
manual_test_results = test_df.merge(manual_predictions, on='Date', how='left')

# Save manual threshold results
manual_results_csv = os.path.join(OUT_DIR, f"manual_threshold_results_{name}_{TRAIN_START.replace('-','')}_{TRAIN_END.replace('-','')}.xlsx")
_save_csv(manual_test_results, manual_results_csv)

# Calculate metrics for manual threshold
print("\nMANUAL THRESHOLD PERFORMANCE:")
for horizon in trained_models.keys():
    for model_name in trained_models[horizon].keys():
        prob_col = f"Manual_{horizon.replace('-','')}_{model_name.replace(' ', '_')}_Prob"
        alert_col = f"Manual_{horizon.replace('-','')}_{model_name.replace(' ', '_')}_Alert"
        true_col = f'Event_Next_{horizon.split("-")[0]}D'
        
        if prob_col in manual_test_results.columns and true_col in manual_test_results.columns:
            y_true = manual_test_results[true_col]
            y_pred = manual_test_results[alert_col]

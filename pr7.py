...
def calculate_coverage_metrics(y_true_daily, y_pred_alerts, horizon_days):
    ...
    """
    # Window size is horizon + 1 (e.g. 1-day horizon means D-1 and D, so 2 days)
    window_size = horizon_days + 1
    
    # Calculate rolling sum of alerts looking back 'window_size' days
    alert_series = pd.Series(y_pred_alerts)
    active_alerts_sum = alert_series.rolling(window=window_size, min_periods=1).sum()
    
    # If any alert in window, Prediction = 1
...
def optimize_threshold_for_coverage(y_true_daily, y_proba_smoothed, horizon_days):
    """
    NEW OPTIMIZATION:
    Iterates through potential thresholds (0.05 to 0.95) and selects the one
    that yields the highest F1 score using the COVERAGE logic.
    """
    best_threshold = 0.5
    best_f1 = -1
    ...
    for thresh in thresholds:
        # 1. Generate Binary Alerts at this threshold
...
# --- NEW: Calculate Daily Article Counts and Z-Score Globally ---
# We calculate this once here to use as a spike feature
...
# Z-Score over 30-day window to detect spikes
...
for scenario_name, scenario_formula in scenarios.items():
    for aggregation in aggregations:
        ...
        try:
            ...
            all_results.append({
                "scenario": scenario_name, "aggregation": aggregation, "horizon": "N/A", "model": "N/A",
                "status": "error", "error": f"Formula Eval Error: {str(e)}"
            })
            continue
        ...
        # --- FEATURE LIST UPDATE: Added Article_Count_ZScore ---
        ...
        base_models = {
            ...
            "Isolation Forest": IsolationForest(contamination=0.1, random_state=42) # New Anomaly Detector
        }
        ...
        for target_col, horizon in [("Event_Next_1D", "1-day"), ("Event_Next_3D", "3-day"), ("Event_Next_7D", "7-day")]:
            ...
            if len(np.unique(y_train)) < 2: 
                all_results.append({
                    "scenario": scenario_name, "aggregation": aggregation, "horizon": horizon, "model": "N/A",
                    "status": "error", "error": "Train data has only 1 class"
                })
                continue
            
            # --- FEATURE 3: Tune XGBoost for Precision ---
            if "XGBoost" in base_models and len(np.unique(y_train)) >= 2:
                pos_ratio = y_train.sum() / len(y_train)
                if pos_ratio > 0:
                    # Multiplier 0.2 forces model to be conservative (Precision > Recall)
            ...
            for model_name, base_model in base_models.items():
                try:
                    ...
                    if model_name == "Isolation Forest":
                        ...
                        model.fit(X_train)
                        # Decision function: lower = anomaly. We negate so higher = anomaly (signal)
                    ...
                    # Calculate AUC
                    try:
                        auc_val = roc_auc_score(y_test_target, y_test_smooth)
                    except: auc_val = 0.5
...
# 5. Alert Generation Functions (With ALL columns)
def generate_alert_table_full(run_data_cache, scenario, aggregation, horizon, model_name, threshold, output_dir, file_prefix):
    ...
    if isinstance(model_obj, tuple):
        # Could be LR pipeline or Isolation Forest tuple
        tag = model_obj[0]
        if tag == "pipeline":
            ...
            probs = np.zeros(len(test_df)) # Fallback
    ...
    # Add Prediction Columns to the FULL dataframe
...
# --- CONFIGURE YOUR CUSTOM RUN HERE ---
# Defaults to what is likely your best performing setup now: 7-day risk, XGBoost (High Precision)
CUSTOM_SCENARIO = "NumSourcesXNumMentionsXNumArticles_Zscore"         
CUSTOM_AGG      = "sum"                  

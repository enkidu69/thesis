...
def get_user_counterparts():
    """
    Get user input for which counterparts to analyze
    """
    print("\n" + "="*60)
    print("COUNTERPART SELECTION")
    print("="*60)
    
    # Default selection of interesting counterparts
    ...
    print("3. Show all available counterparts first")
    
    choice = input("\nChoose option (1/2/3): ").strip()
    
    if choice == "1":
        return default_counterparts
    elif choice == "2":
        custom_input = input("Enter country codes (ISO2) separated by commas (e.g., RU,SY,YE,KP,VE): ").strip()
        if custom_input:
            counterparts = [code.strip().upper() for code in custom_input.split(',')]
            # Validate country codes
            valid_counterparts = []
            for code in counterparts:
                if code in FIPS_TO_ISO2.values():
                    valid_counterparts.append(code)
                else:
                    print(f"Warning: '{code}' is not a valid ISO2 country code")
            
            if valid_counterparts:
                return valid_counterparts
            else:
                print("No valid country codes found. Using defaults.")
                return default_counterparts
        else:
            print("No input provided. Using defaults.")
            return default_counterparts
    else:  # Option 3 or any other input
        return None  # Will show available counterparts later
        ...
        if os.path.exists(chunk_path):
            ...
            try:
                ...
                if len(df_chunk) > 0:
                    ...
                    df_filtered = filter_and_convert_columns(df_chunk, focal_countries)
                    
                    # Save chunk to file
                    df_filtered.to_pickle(chunk_path)
                    chunk_files.append(chunk_path)
                    
                    print(f"  Saved chunk with {len(df_filtered):,} events to {chunk_filename}")
                else:
...
def process_chunk_files(chunk_files, focal_countries, event_codes=None, selected_counterparts=None):
    """
        ...
        for focal_country in focal_countries:
            ...
            if len(df_filtered) > 0:
                ...
                def get_relationship_pair(row):
                    ...
                    # Collect all counterparts for reporting
                    if pd.notna(counterpart) and counterpart != '':
                        all_counterparts.add(counterpart)
    ...
    if all_country_data:
        ...
        # Show available counterparts
        print(f"\nFound {len(all_counterparts)} unique counterpart countries")
        
        # Filter by selected counterparts if provided
        if selected_counterparts:
            # Create filter for selected counterparts
            ...
            print(f"Filtered to {len(combined_df):,} events with selected counterparts (from {before_filter:,})")
        
        return combined_df, sorted(list(all_counterparts))
    else:
        return pd.DataFrame(), []
...
def create_comparison_pdf_report(daily_scores, focal_countries, selected_counterparts, year, output_dir='reports'):
    """
    Create PDF report comparing two focal countries against SELECTED counterparts
    """
    ...
    # Use selected counterparts
    top_counterparts = selected_counterparts
    
    print(f"Creating report for {len(top_counterparts)} selected counterparts: {', '.join(top_counterparts)}")
    
    # Create a distinct color palette for counterparts
    colors = plt.cm.Set3(np.linspace(0, 1, len(top_counterparts)))
    color_dict = dict(zip(top_counterparts, colors))
    
    # Different line styles for focal countries
    line_styles = {'solid': '-', 'dashed': '--', 'dotted': ':', 'dashdot': '-.'}
    
    with PdfPages(pdf_path) as pdf:
        plt.style.use('default')
        
        # PAGE 1: Overview - Comparison with selected counterparts
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle(f'Bilateral Analysis: {" vs ".join(focal_countries)} with Selected Counterparts ({year})', 
                    fontsize=16, fontweight='bold', y=0.98)
        
        # Plot 1: Daily Negativity Score Comparison
        for counterpart in top_counterparts:
            for i, focal_country in enumerate(focal_countries):
                ...
                if not rel_data.empty:
                    linestyle = list(line_styles.values())[i % len(line_styles)]
                    marker = ['o', 's', '^', 'D'][i % 4]
                    axes[0, 0].plot(rel_data['Date'], rel_data['Negativity_Score'], 
                                   label=f'{focal_country}-{counterpart}', 
                                   color=color_dict[counterpart], linestyle=linestyle, linewidth=2,
        ...
        # Plot 2: Tone-Article Impact Index
        for counterpart in top_counterparts:
            for i, focal_country in enumerate(focal_countries):
                rel_data = daily_scores[
                    (daily_scores['FocalCountry'] == focal_country) & 
                ...
                if not rel_data.empty:
                    linestyle = list(line_styles.values())[i % len(line_styles)]
                    marker = ['o', 's', '^', 'D'][i % 4]
                    axes[0, 1].plot(rel_data['Date'], rel_data['Tone_Article_Index'], 
                                   label=f'{focal_country}-{counterpart}', 
                                   color=color_dict[counterpart], linestyle=linestyle, linewidth=2,
                                   marker=marker, markersize=4)
        
        axes[0, 1].set_title('Tone-Article Impact Index\n(AvgTone × NumArticles)', 
                            fontsize=12, fontweight='bold')
        axes[0, 1].set_ylabel('Impact Index', fontsize=10)
        axes[0, 1].xaxis.set_major_locator(plt.MaxNLocator(8))
        axes[0, 1].tick_params(axis='x', rotation=45)
        axes[0, 1].grid(True, alpha=0.3)
        axes[0, 1].legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=8)
        
        # Plot 3: Negativity-Article Impact Index
        for counterpart in top_counterparts:
            for i, focal_country in enumerate(focal_countries):
                rel_data = daily_scores[
                    (daily_scores['FocalCountry'] == focal_country) & 
                ...
                if not rel_data.empty:
                    linestyle = list(line_styles.values())[i % len(line_styles)]
                    marker = ['o', 's', '^', 'D'][i % 4]
                    axes[1, 0].plot(rel_data['Date'], rel_data['Negativity_Article_Index'], 
                                   label=f'{focal_country}-{counterpart}', 
                                   color=color_dict[counterpart], linestyle=linestyle, linewidth=2,
                                   marker=marker, markersize=4)
        
        axes[1, 0].set_title('Negativity-Article Impact Index\n(Negativity × NumArticles)', 
                            fontsize=12, fontweight='bold')
        ...
        # Plot 4: Event Count by Counterpart (Stacked bar)
        event_counts = {}
        for counterpart in top_counterparts:
            event_counts[counterpart] = {}
            for focal_country in focal_countries:
                rel_data = daily_scores[
                    (daily_scores['FocalCountry'] == focal_country) & 
                    (daily_scores['RelationshipPair'] == f"{focal_country}-{counterpart}")
                ]
                event_counts[counterpart][focal_country] = rel_data['Daily_EventCount'].sum() if not rel_data.empty else 0
        
        # Create stacked bar chart
        bottom = np.zeros(len(top_counterparts))
        for i, focal_country in enumerate(focal_countries):
            counts = [event_counts[counterpart][focal_country] for counterpart in top_counterparts]
            axes[1, 1].bar(top_counterparts, counts, bottom=bottom, label=focal_country, 
                          color=['red', 'blue'][i % 2], alpha=0.7)
            bottom += counts
        
        axes[1, 1].set_title('Total Event Count by Counterpart\n(Stacked by Focal Country)', 
                            fontsize=12, fontweight='bold')
        ...
        # PAGE 2: Individual counterpart analysis
        ...
        for page in range(total_pages):
            ...
            fig.suptitle(f'Individual Counterpart Analysis - Page {page+1}', 
                        fontsize=16, fontweight='bold', y=0.98)
            ...
            for i, counterpart in enumerate(page_counterparts):
                ...
                if row < 2:
                    ...
                    axes[row, col].set_title(f'{counterpart}\nTone-Article Impact Index', 
...
if __name__ == "__main__":
    ...
    # GET USER COUNTERPART SELECTION
    selected_counterparts = get_user_counterparts()
    
    chunk_files = []
    
    try:
        print(f"\nStarting incremental download and processing for {YEAR}...")
        ...
        combined_df, all_counterparts = process_chunk_files(chunk_files, FOCAL_COUNTRIES, EVENT_CODES, selected_counterparts)
        
        if combined_df.empty:
            print("No data after processing! Exiting.")
            exit()
        
        # If user chose to see available counterparts first
        if selected_counterparts is None:
            print(f"\nAvailable counterparts ({len(all_counterparts)} countries):")
            print(", ".join(sorted(all_counterparts)))
            
            custom_input = input("\nEnter country codes to analyze (ISO2) separated by commas: ").strip()
            if custom_input:
                selected_counterparts = [code.strip().upper() for code in custom_input.split(',')]
                # Re-filter the data with selected counterparts
                counterpart_filter = False
                for focal_country in FOCAL_COUNTRIES:
                    for counterpart in selected_counterparts:
                        relationship = f"{focal_country}-{counterpart}"
                ...
                before_filter = len(combined_df)
                combined_df = combined_df[counterpart_filter].copy()
                print(f"Filtered to {len(combined_df):,} events with selected counterparts")
            else:
                print("No counterparts selected. Using default selection.")
                selected_counterparts = ['RS']
        
        print(f"\nAnalyzing {len(selected_counterparts)} counterparts: {', '.join(selected_counterparts)}")
        ...
        print("\nCreating PDF report...")
        pdf_path = create_comparison_pdf_report(daily_scores, FOCAL_COUNTRIES, selected_counterparts, YEAR)
        ...
        # Save daily scores
        scores_filename = f'{"_".join(FOCAL_COUNTRIES)}_scores_{YEAR}_{random_str}.csv'
